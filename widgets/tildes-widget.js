...
*
* SETTINGS
* You can change the four values bellow this comment.
...
* `@userName` - anything starting with a `@` is a user mame
* `` - left blank, the widget displays a post from the home page
* `~group,@user` - the identifiers can be seporated by commas with NO spaces between to get a random post from one of the options
...
const radomQuery = getRandomItem(params)
const data = await getTildesData(radomQuery)
...
if (data.length === 0) {
  presentError(`No posts found on page: ${radomQuery}`)
  return
}
